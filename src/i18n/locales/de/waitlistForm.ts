export const waitlistForm = {
  label: 'Warteliste',
  subscribedLabel: 'Angemeldet',
  successMessage: 'Prüfen Sie Ihren Posteingang — eine Bestätigungs-E-Mail ist unterwegs. Klicken Sie den Link, um Ihren Platz auf der Warteliste zu bestätigen.',
  getNotified: 'Beim Launch benachrichtigt werden',
  sending: 'Wird gesendet…',
  notifyMe: 'Benachrichtigen',
  namePlaceholder: 'Dein Name',
  emailPlaceholder: 'deine@email.de',
  errorGeneric: 'Etwas ist schiefgelaufen. Bitte versuchen Sie es erneut.',
  errorNetwork: 'Der Anmeldedienst ist nicht erreichbar. Bitte versuchen Sie es später erneut.',
  disclaimer: 'Double-Opt-in. Kein Spam. Jederzeit abmeldbar.',
};
